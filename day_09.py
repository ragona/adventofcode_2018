...
class MarbleGame:
    ...
    def __repr__(self):
        return f"MarbleGame({' '.join([str(m) for m in self.marbles])})"

    @property
    def done(self):
        return self.marble >= self.last_marble
    ...
    def turn(self):
        if self.done:
            raise Exception("Tried to play a done game")

        new_marble = self.marble + 1

        if new_marble % 23 == 0:
            player = new_marble % self.num_players

            self.marbles.rotate(7)
            self.players[player] += self.marbles.pop()
            self.players[player] += new_marble
            self.marbles.rotate(-1)
        else:
            self.marbles.rotate(-1)
            self.marbles.append(new_marble)

        self.marble = new_marble
...
def main():
    game = MarbleGame(424, 71144)
    game.play()

    print("Part 1:", game.winning_score)

    game = MarbleGame(424, 71144 * 100)
    game.play()

    print("Part 2:", game.winning_score)
...
class TestMarbles(unittest.TestCase):
    ...
    """
    def setUp(self):
        self.game = MarbleGame(9, 25)

    def test_marbles(self):
        self.game.play()
        assert self.game.winning_score == 32
